...
{ 
   ...
   */
  //Create stuffs and make stuffs 0 too.
  int rem_bt[n];
  int t = 0;
  //Store wait times of processes
  for(int i = 0; i<n; i++){
    rem_bt[i] = plist[i].bt;
    plist[i].wt = 0;
  }
  //Traverse all processes while all process are done.
  bool done = false;
  while(!done){
    done = true;
    for(int i=0;i<n; i++){
...
{ 
  ...
  //Finding the process with min remaining time at every time lap.
  while(p_count < n){
    int min = 0;
    for(int i=1; i<n; i++){
    ...
    if (rem_bt[min] == 0){
     ...
     //Completion time of *current process = current_time +1;*
     complete_time = cur_time + 1;
      
     //Calculate waiting time for each completed process. *wt[i]= Completion time - arrival_time-burst_time*
     plist[min].wt = complete_time - plist[min].bt;
     rem_bt[min]=9999;
